...
export const createGoal = createAsyncThunk("goals/create", async (payload, thunkAPI) => {
    ...
    try {
        ...
        if (!goalID) {
            const url = `${process.env.REACT_APP_SERVER_URL}/goals/create?email=${encodeURIComponent(email)}&name=${encodeURIComponent(goalName)}`;
            ...
            dispatch(setSelectedGoal(response.data))
        } else {
            const url = `${process.env.REACT_APP_SERVER_URL}/goals/update?id=${encodeURIComponent(goalID)}&name=${encodeURIComponent(goalName)}`;
...
export const deleteGoal = createAsyncThunk("goals/delete", async (payload, thunkAPI) => {
    try {
        ...
        const url = `${process.env.REACT_APP_SERVER_URL}/goals/delete?id=${encodeURIComponent(goalID)}`;
